...
def lets_roll(input_filename,output_filename):
	with open(input_filename,'r') as input: 
		if os.path.exists(input_filename+'.csv'):
			os.remove(input_filename+'.csv')

		with open(output_filename+'output.csv','a') as output:
...
def main():
	parser = optparse.OptionParser('%prog ' + '-i <input_filename> -o <output_filname>')
	parser.add_option('-i' ,dest ='input_filename', type='string', help = 'Specify input filepath')
	parser.add_option('-o' ,dest ='output_filename', type='string', help = 'Specify output filepath exculding the filename')
	
	(options, args) = parser.parse_args()
	input_filename = options.input_filename
	output_filename=options.output_filename
	
	checkfilepath(input_filename,output_filename)
	lets_roll(input_filename,output_filename)

def checkfilepath(input_filename,output_filename):
